...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (settingsError || !emailSettings?.smtp_user || !emailSettings?.smtp_password) {
    ...
    const fromName = emailSettings.from_name || 'Legal Case Manager';
    const fromEmail = emailSettings.from_email || emailSettings.smtp_user;

    // Create SMTP client
    const client = new SMTPClient({
      connection: {
        hostname: emailSettings.smtp_host || 'smtp.gmail.com',
    ...
    await client.send({
      ...
      content: 'আপনার সাবস্ক্রিপশন রিনিউ করুন',
      html: htmlContent,
    });
